...
public class SetupMasterTestEnv {
  ...
  public void setCfgAndStartMaster() throws TTransportException, IOException {
    ...
    FlumeConfiguration.get().set(
        FlumeConfiguration.WEBAPP_ROOT_MASTER,
        getClass()
            .getClassLoader()
            .getResource(
                "flume-config-web-"
                    + System.getProperty("flume.test.master.version") + ".war")
            .getFile());
    tmpdir = FileUtil.mktempdir();
    FlumeConfiguration.createTestableConfiguration();
    System.setProperty("flume.home", new File(".").getAbsolutePath());
    FlumeConfiguration.get().set(FlumeConfiguration.WEBAPP_ROOT_MASTER,
        "src/main/webapp");
